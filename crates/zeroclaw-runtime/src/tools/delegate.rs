    ...
    async fn execute_agentic_with_admission(
        ...
        let mut sub_tools: crate::tools::scoped::ScopedToolRegistry = match target_mode {
            ...
            DelegateExecutionMode::Bounded => {
                ...
                // parent's own delegate instance, filtered by the caller's own
                // admission. Every per-category substitution below draws its
                // candidate names from this set, and it is ALSO handed
                // unchanged to `sub_delegate_tool` further down as ITS
                // `parent_tools`: a further bounded hop re-substitutes its own
                // memory / filesystem / workspace-bound / identity-bound /
                // channel / autonomy tools by name exactly as this hop does,
                // rather than inheriting this hop's already-substituted
                // instances. The read guard is scoped to this block so it
                // drops BEFORE the `assemble().await` below - a parking_lot
                // guard held across an await would make the delegate future
                // `!Send`.
                ...
                // delegating parent's - reusing the parent's own instance here
                // would be a confused-deputy shape. Depth ownership: this
                // construction is the single site that increments depth for a
                // logical hop (`self.depth + 1`); the background/parallel
                // wrappers inherit their depth verbatim. The carried ceiling is
                // this tool's effective ceiling tightened (min) by the target's
                // own profile cap, so a parent's cap binds the whole subtree
                ...
                .then(|| {
                    ...
                    Box::new(DelegateTool {
                        ...
                        // Same live handles as the delegating turn: a further
                        // sub-delegation is still within the same channel
                        // route, not a new one to rebuild against.
                        channel_handles: self.channel_handles.clone(),
                        task_control_plane: nested_task_control_plane,
                    }) as Box<dyn Tool>
                });

                // Build the bounded tool set: `bounded_base_tools`, with the
                // target's own rebuilt memory / filesystem / workspace-bound /
                // identity-bound / channel / autonomy instances substituted in
                // by name, then `sub_delegate_tool` appended - it takes the
                // retained delegation slot the parent's own instance used to
                // fill implicitly before being stripped above.
                ...
                // offered, not the authority behind it. Two entries in THIS
                // registry cannot survive that projection intact, for two
                // different reasons - both excluded below:
                //
                // - `BoundedSopApproveDenied` reports the same `name()` as the
                //   real `SopApproveTool` it stands in for, deliberately, so the
                //   model sees the schema instead of a missing tool. Every other
                //   denied tool is instead simply absent from this registry, so
                //   this is the one name that needs an explicit exclusion rather
                //   than falling out of the set on its own (see its own doc
                //   comment).
                // - `sub_delegate_tool`, when present, is the ONLY source of
                //   `Self::NAME` ("delegate") in this registry - the caller's own
                //   instance is unconditionally stripped from `bounded_base_tools`
                //   above and never substituted back. Its authority is tightened
                //   to THIS hop (`depth + 1`, `tightened_max_depth` intersecting
                //   the caller's own binding cap) and lives only in that one
                //   instance. A name-only ceiling cannot carry that tightening: a
                //   cron job stored without an explicit `allowed_tools` list caps
                //   to the whole ceiling, and a scheduler replay of that job
                //   builds a FRESH, ordinary `DelegateTool` for the owning
                //   agent via `all_tools_with_runtime`'s `DelegateTool::new_with_options`
                //   (`depth: 0`, no caller cap to intersect with) and retains it
                //   by matching the stored name - regaining exactly the depth
                //   binding this hop existed to enforce on its whole subtree.
                //
                // Both rebuild a normal, unbounded registry later and retain by
...
mod tests {
    ...
    // This name passes bounded admission so the probe isolates the approval gate.
    const APPROVAL_PROBE_TOOL_NAME: &str = "calculator";
    ...
    async fn bounded_delegate_full_fixture_multi(
        ...
        .expect("caller tool registry builds")
        .tools;
    ...
    fn bounded_subdelegation_tool(config: &Arc<Config>) -> DelegateTool {
        ...
        DelegateTool::new(config.agents.clone(), None, Arc::clone(&caller_policy))
            ...
            .with_runtime_profiles(config.runtime_profiles.clone())
            .with_parent_tools(Arc::new(RwLock::new(vec![
                // Not `EchoTool`: this fixture sets `.with_root_config`, which
                // turns on `deny_unclassified_reuse` in the bounded rebuild -
                // an unclassified test double would be denied there exactly
                // like any other unproven tool, which is the property under
                // test elsewhere, not here. `calculator` is a real production
                // tool already listed in `SAFE_FOR_BOUNDED_REUSE`.
...
mod bounded_rate_limit_is_the_targets_own {
    ...
    /// Was `a_target_with_budget_is_not_stopped_by_an_exhausted_caller`, and
    /// asserted the opposite of what this now checks.
    ///
    /// `policy_for_target` (`delegate.rs:677-679`) tightens
    /// `max_actions_per_hour` to `target.max_actions_per_hour.min(caller's)` -
    /// the same "a parent's cap binds its whole subtree" contract this file
    /// already enforces for delegation depth (`tightened_max_depth`). A
    /// target with a looser profile ceiling can no longer outspend the
    /// caller's own budget by being delegated to, which is exactly the
    /// scenario this test used to require the OPPOSITE of. `min(0, 100) = 0`,
    /// so the call must now be refused, not admitted.
    #[tokio::test]
    async fn a_target_with_room_is_still_bound_by_an_exhausted_callers_ceiling() {
        let result = tool_result_with_budgets(0, 100).await;

        assert!(
            result.contains(RATE_LIMIT_MARKER),
            "a target with its own room acted past the caller's exhausted ceiling, which \
             `policy_for_target`'s budget intersection exists to prevent; {result}"
        );
        assert!(
            !result.contains(INNER_REACHED_MARKER),
            "the inner search tool was reached, so the wrapper admitted a call the caller's \
             own ceiling did not have room for; {result}"
        );
    }
}
